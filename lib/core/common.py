...
def readInput(message, default=None, checkBatch=True, boolean=False):
	"""
	Reads input from terminal. Modified to prompt for user input in batch mode if an answer is provided without a value.
	"""
	retVal = None
		...
		for item in conf.answers.split(","):
			...
			if question.lower() in message.lower():
				if given_answer == "":  # Answer provided without a value
					...
					given_answer and retVal is None
				):  # Only set retVal if it hasn't been set by user input
					retVal = getUnicode(given_answer, UNICODE_ENCODING)

	if message and IS_TTY and retVal is None:
		message = "\r%s" % message
